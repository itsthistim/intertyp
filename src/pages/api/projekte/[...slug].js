  ...
  try {
    ...
    if (image.url.startsWith("/")) {
      ...
      if (fs.existsSync(imagePath)) {
        ...
        console.warn(
          `Local image for dimension check not found: ${imagePath} (URL: ${image.url})`,
        );
      }
      ...
      if (response.ok) {
        ...
        console.warn(
          `Failed to fetch remote image for dimension check: ${image.url}, status: ${response.status}`,
        );
      }
    }
    ...
    if (dimensions) {
      ...
      try {
        await db.query(`UPDATE image SET width = ?, height = ? WHERE url = ?`, [
          image.width,
          image.height,
          image.url,
        ]);
        ...
        console.error(
          `Error updating image dimensions in database for ${image.url}:`,
          dbError.message,
        );
      }
    } else {
...
async function getGallery(galleryId) {
  ...
  const [galleryImageRows] = await db.query(
    `SELECT * FROM gallery_image WHERE gallery_id = ?`,
    [galleryId],
  );
    ...
    gallery.images = await Promise.all(
      galleryImageRows.map(async (galleryImage) => {
        const [imageRows] = await db.query(
          `SELECT url, alt, width, height FROM image WHERE image_id = ?`,
...
async function findOrCreateLink(link, slug) {
  ...
  const [newLinkResult] = await db.query(
    `INSERT INTO link (slug, name) VALUES (?, ?)`,
    [slug, link?.name || null],
  );
  if (newLinkResult.insertId) {
    const [newLinkRows] = await db.query(
      `SELECT * FROM link WHERE link_id = ?`,
      [newLinkResult.insertId],
    );
...
async function findOrCreateImage(image) {
  ...
  let [imageRows] = await db.query(`SELECT * FROM image WHERE url = ?`, [
    image.url,
  ]);
  ...
  if (newImageResult.insertId) {
    const [newImageRows] = await db.query(
      `SELECT * FROM image WHERE image_id = ?`,
      [newImageResult.insertId],
    );
...
async function findOrCreateGallery(gallery) {
  ...
  const [newGalleryResult] = await db.query(
    `INSERT INTO gallery (title) VALUES (?)`,
    [gallery.title],
  );
      ...
      await Promise.all(
        gallery.images.map(async (img) => {
          ...
          await db.query(
            ...
            [newGallery.gallery_id, image.image_id],
          );
        }),
      );
...
async function updateProjectGallery(gallery, existingGalleryId) {
  if (!gallery)
    return { galleryId: existingGalleryId, oldGalleryToCleanup: null };
  ...
  const oldGalleryToCleanup =
    existingGalleryId && existingGalleryId !== newGalleryId
      ? existingGalleryId
      : null;
...
function buildUpdateQuery(fields) {
  const { title, description, project_date, link, cover_image, gallery } =
    fields;
  let updateFields = [];
  let updateValues = [];
...
async function cleanupOldGallery(oldGalleryId) {
  ...
  try {
    ...
    if (galleryRefs[0].count === 0) {
      ...
      await db.query(`DELETE FROM gallery_image WHERE gallery_id = ?`, [
        oldGalleryId,
      ]);
      await db.query(`DELETE FROM gallery WHERE gallery_id = ?`, [
        oldGalleryId,
      ]);
    }
  } catch (cleanupError) {
    ...
    console.warn(
      `Warning: Could not clean up old gallery ${oldGalleryId}:`,
...
async function formatProjectResponse(projectData) {
  return {
    ...
    cover_image: await getCoverImage(
      projectData.cover_url,
      projectData.cover_alt,
    ),
...
export async function GET({ request, params }) {
  ...
  try {
    const [projectRows] = await db.query(
      `SELECT * FROM project_v WHERE link_slug = ? LIMIT 1`,
      [slug],
    );
    ...
    let project = {
      ...
      cover_image: await getCoverImage(
        projectRow.cover_url,
        projectRow.cover_alt,
      ),
...
export async function POST({ request, params }) {
  ...
  try {
    ...
    const [existingProject] = await db.query(
      `SELECT * FROM project_v WHERE link_slug = ?`,
      [slug],
    );
    ...
    if (newProject[0].project_id) {
      const [createdProjectRows] = await db.query(
        `SELECT * FROM project_v WHERE id = ?`,
        [newProject[0].project_id],
      );
      if (createdProjectRows.length > 0) {
        ...
        return new Response(
          JSON.stringify({
            ...
            cover_image: await getCoverImage(
              createdProject.cover_url,
              createdProject.cover_alt,
            ),
...
export async function PUT({ request, params }) {
  ...
  try {
    ...
    const linkId = await updateProjectLinks(
      link,
      slug,
      existingProjectData.link_id,
    );
    const coverImageId = await updateProjectCoverImage(
      cover_image,
      existingProjectData.cover_id,
    );
    const { galleryId, oldGalleryToCleanup } = await updateProjectGallery(
      gallery,
      existingProjectData.gallery_id,
    );
    ...
    if (updateFields.length > 0) {
      ...
      await db.query(
        `UPDATE project SET ${updateFields.join(", ")} WHERE project_id = ?`,
        updateValues,
      );
    }
    ...
    if (updatedProjectRows.length > 0) {
      ...
      return new Response(
        JSON.stringify({ error: "Failed to retrieve updated project." }),
        {
          status: 500,
          headers: { "Content-Type": "application/json" },
        },
  ...
  try {
    ...
    if (galleryId) {
      try {
        ...
        // Log the error but don't fail the entire request
        console.warn(
          `Warning: Could not clean up gallery ${galleryId}: ${cleanupError.message}`,
        );
      }
    }
    ...
    if (coverImageId) {
      try {
        ...
        if (imageRefs[0].count === 0) {
          // Safe to delete - no references exist
          await db.query(`DELETE FROM image WHERE image_id = ?`, [
            coverImageId,
          ]);
        }
      } catch (cleanupError) {
        // Log the error but don't fail the entire request
        console.warn(
          `Warning: Could not clean up cover image ${coverImageId}: ${cleanupError.message}`,
        );
      }
    }
    ...
    if (linkId) {
      try {
        ...
        // Log the error but don't fail the entire request
        console.warn(
          `Warning: Could not clean up link ${linkId}: ${cleanupError.message}`,
        );
      }
    }
