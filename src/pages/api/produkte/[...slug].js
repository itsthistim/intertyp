...
async function findOrCreateImage(image) {
  ...
  const [newImageResult] = await db.query(
    ...
    [image.url, image.alt === null || image.alt === undefined ? "" : image.alt],
  );
...
export async function GET({ request, params }) {
  ...
  try {
    const [productRows] = await db.query(
      `SELECT * FROM product_v WHERE link_slug = ? LIMIT 1`,
      [slug],
    );
    ...
    product.cover_image = await getCoverImage(
      productRow.cover_url,
      productRow.cover_alt,
    );
...
export async function POST({ request, params }) {
  ...
  if (!title) {
    return new Response(
      JSON.stringify({ error: "Missing required fields: title is required." }),
      {
        status: 400,
        headers: { "Content-Type": "application/json" },
      },
  ...
  try {
    ...
    const [existingProduct] = await db.query(
      `SELECT * FROM product_v WHERE link_slug = ?`,
      [slug],
    );
    ...
    const [newProduct] = await db.query(
      `INSERT INTO product (title, description, cover_image_id, link_id, gallery_id) VALUES (?, ?, ?, ?, ?) RETURNING product_id`,
      [
        title,
        description || null,
    ...
    if (newProduct[0].product_id) {
      const [createdProductRows] = await db.query(
        `SELECT * FROM product_v WHERE id = ?`,
        ...
        return new Response(
          JSON.stringify({
            ...
            cover_image: await getCoverImage(
              createdProduct.cover_url,
              createdProduct.cover_alt,
            ),
  ...
  try {
    ...
    if (typeof oldGalleryToCleanup !== "undefined") {
      try {
        ...
        console.warn(
          `Warning: Could not clean up old gallery ${oldGalleryToCleanup}: ${cleanupError.message}`,
        );
      }
    }
    ...
    if (updatedProductRows.length > 0) {
      ...
      return new Response(
        JSON.stringify({
          ...
          cover_image: await getCoverImage(
            updatedProduct.cover_url,
            updatedProduct.cover_alt,
          ),
      ...
      return new Response(
        JSON.stringify({ error: "Failed to retrieve updated product." }),
        {
          status: 500,
          headers: { "Content-Type": "application/json" },
        },
